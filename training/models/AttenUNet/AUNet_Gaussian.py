...
class AUNet_Gaussian(nn.Module):
    def __init__(self,in_channels = 6,out_channels = 6,depth = 3,growth_factor = 6,
        ...
        super(AUNet_Gaussian,self).__init__()
        ...
        self.final_mean = nn.Conv2d(prev_channels,out_channels,1,1,0)
        self.final_variance = nn.Conv2d(prev_channels,out_channels,1,1,0)
    ...
    def forward(self,MS,PAN = None):
        ...
        mean = F.relu(self.final_mean(x))
        variance = F.relu(self.final_variance(x))
        return mean, variance


if __name__ == '__main__':
    x = torch.rand([9,7,256,256]).cuda()
    model = AUNet_Gaussian(in_channels=7, out_channels=6, depth=5, spatial_attention="modified", growth_factor=6,
