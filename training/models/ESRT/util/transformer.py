...
class EffAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.reduce = nn.Linear(dim, dim//2, bias=qkv_bias) #Reduction Layer
        self.qkv = nn.Linear(dim//2, dim//2 * 3, bias=qkv_bias) # 3 ELEMENTS
    ...
    def forward(self, x):
        ...
        print(x.shape)
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4) # RESHAPED AND PERMUTED TO B x m x N x C/m
...
class MLABlock(nn.Module):
    def __init__(
        ...
        self.mlp = Mlp(in_features=self.dim, hidden_features=dim//4, act_layer=act_layer, drop=drop)
    ...
    def forward(self, x):
        ...
        # print(x.shape)
        print(f"Before patch extraction {x.shape}")
