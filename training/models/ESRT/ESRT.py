...
from training.models.ESRT.util.tools import extract_image_patches, \
    reduce_mean, reduce_sum, same_padding, reverse_patches
import training.models.ESRT.common as common
from training.models.ESRT.util.transformer import drop_path, DropPath, PatchEmbed, Mlp, MLABlock
import math
...
class ESRT(nn.Module):
    def __init__(self, in_channels = 3, out_channels = 6, upscale=4, pan_fusion = False, n_heads = 8, conv=common.default_conv):
        ...
        # define pan-fusion module
        if pan_fusion:
            self.pan_head = conv(1, n_feats // 2, kernel_size)
            modules_ms_features = [
                common.Upsampler(conv, scale, n_feats, act=False),
                conv(n_feats, n_feats // 2, kernel_size)
            ]
            self.ms_features = nn.Sequential(*modules_ms_features)
            self.fusion = conv(n_feats, out_channels, kernel_size)
        ...
        self.up = nn.Sequential(common.Upsampler(conv, scale, n_feats, act=False),
                                BasicConv(n_feats, out_channels, 3, 1, 1))
        self.reduce = conv(n_blocks * n_feats, n_feats, kernel_size)

    def forward(self, MS, pan = None, test=False):
        if (not pan is None) and (not self.pan_fusion):
            x1 = torch.cat([MS,pan], dim=1)
        else:
            x1 = MS
        x1 = self.head(x1)
        res2 = x1
        ...
        if not self.pan_fusion:
            x1 = self.tail(res1)
            x1 = self.up(res2) + x1
            return x1
        else:
            # REPLACING THE TAIL MODULE LAYERS TO ACCOMODATE PAN FUSION
            pan_head = self.pan_head(pan)
            ms_features = self.ms_features(res1)
            pan_head = F.interpolate(pan_head, size = ms_features.shape[2:],mode = "bilinear")
            # FUSE THE PAN AND MS FEATURES AND APPLY FINAL RESIDUAL
            x1 = self.fusion(torch.cat([ms_features, pan_head],dim = 1))
            x1 = self.up(res2) + x1
            return x1
...
if __name__ == '__main__':
    model = ESRT(in_channels=7,upscale=1, pan_fusion= False).cuda()
    ms = torch.rand([1,6,128,128]).cuda()
